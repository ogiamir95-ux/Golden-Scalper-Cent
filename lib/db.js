...
// Ambil SEMUA baris akun sekaligus (untuk ringkasan Admin Panel — satu
// query, bukan N+1 per user). Return Map<accountLogin, row> supaya lookup
// di endpoint pemanggil cepat (O(1) per user).
export async function listAccountsMap(accountLogins) {
  const uniq = [...new Set((accountLogins || []).filter(Boolean).map(String))];
  if (uniq.length === 0) return new Map();
  const { data, error } = await supabase
    .from("accounts")
    .select("*")
    .in("account_login", uniq);
  if (error) throw error;
  const map = new Map();
  for (const row of data || []) map.set(String(row.account_login), row);
  return map;
}

// Simpan tanggal expiry lisensi yang baru saja di-generate admin — sebagai
// FALLBACK untuk ditampilkan di Admin Panel selama EA belum pernah/belum
// sempat sync ulang. Begitu EA online & sync, tanggal dari `state.
// licenseExpiry` (dikirim EA real-time) yang jadi sumber utama di UI.
// Tidak menyentuh/menimpa kolom `state` atau `license_key_hash` — kode
// lisensi ASLI tetap tidak pernah disimpan di server.
export async function saveLicenseExpiryFallback(accountLogin, expiresAtDateStr) {
  const { error } = await supabase
    .from("accounts")
    .upsert(
      { account_login: String(accountLogin), license_expires_at: expiresAtDateStr },
      { onConflict: "account_login" }
    );
  if (error) throw error;
}
...
// Cari user web berdasarkan Akun ID MT5 yang sudah dikaitkan — dipakai
// saat generate lisensi untuk tahu ke email siapa kode harus dikirim.
// Kalau satu account_login dikaitkan ke beberapa user (seharusnya tidak
// terjadi di alur normal), ambil yang paling baru dibuat.
export async function getWebUserByAccountLogin(accountLogin) {
  const { data, error } = await supabase
    .from("web_users")
    .select("id,email,account_login,role,status")
    .eq("account_login", String(accountLogin))
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  if (error) throw error;
...
// Hapus PERMANEN satu akun web (baris `web_users`) — dipakai admin untuk
// mencabut akses sepenuhnya. Setelah ini, email tsb tidak lagi bisa
// login sama sekali; harus daftar ulang dari nol (status kembali "pending"
// dan Akun ID MT5 harus dikaitkan ulang oleh admin).
export async function deleteWebUser(id) {
  const { error } = await supabase.from("web_users").delete().eq("id", id);
  if (error) throw error;
}

// REVOKE REMOTE: dipanggil saat admin klik "Hapus Akun & Lisensi".
//
// PENTING — perilaku ini SENGAJA BUKAN hard-delete langsung terhadap baris
// `accounts`. Validasi lisensi EA dihitung offline dari kode lisensi yang
// tertanam di EA (lihat ValidateLicense() di .mq5), jadi menghapus baris
// begitu saja TIDAK mematikan EA — EA akan tetap validasi sukses secara
// lokal selama expiry di kodenya belum lewat.
//
// Supaya penghapusan dari admin panel benar-benar "mencabut" EA yang
// sedang berjalan, kita tandai baris sbg revoked + titipkan pending_command
// "REVOKE". EA akan mengambil sinyal ini di sync berikutnya (GET
// /api/ea-config atau POST /api/ea-update, keduanya membalas command
// tertunda) dan menjalankan aksi sesuai InpLicenseAction (finish cycle /
// close, stop / remove chart) — sama seperti saat lisensi expired biasa.
//
// Baris `accounts` baru benar-benar dihapus permanen SETELAH EA
// mengonfirmasi bahwa command REVOKE sudah dieksekusi (lihat
// acknowledgeRevocation() dipanggil dari ea-update.js), supaya sinyal
// revoke tidak hilang kalau baris dihapus duluan sebelum EA sempat
// membacanya (mis. EA sedang offline saat admin menghapus akun).
export async function deleteAccountData(accountLogin) {
  if (!accountLogin) return;
  const acc = String(accountLogin);

  // Jurnal boleh langsung dihapus — tidak relevan dgn validasi lisensi EA.
  await supabase.from("journal_entries").delete().eq("account_login", acc);

  const { data: existing } = await supabase
    .from("accounts")
    .select("account_login, license_key_hash")
    .eq("account_login", acc)
    .maybeSingle();

  if (!existing) return; // tidak ada data EA utk akun ini, tidak ada yg perlu di-revoke

  // BLACKLIST PERMANEN: catat hash kode lisensi yang SEDANG dipakai EA
  // saat ini (license_key_hash tersimpan di baris accounts, hasil sync
  // terakhir EA) ke tabel revoked_licenses. Tabel ini TIDAK ikut terhapus
  // walau baris `accounts`-nya nanti dihapus (baik lewat purgeRevokedAccount
  // setelah EA ack, maupun dihapus manual oleh admin di Supabase) — jadi
  // kalau EA yang sama dipasang ulang dengan KODE LISENSI YANG SAMA, sync
  // berikutnya akan tetap ditolak server. Kode lisensi BARU (hash beda)
  // untuk akun yang sama tetap tidak terpengaruh & boleh dipakai lagi.
  if (existing.license_key_hash) {
    const { error: blErr } = await supabase.from("revoked_licenses").upsert(
      {
        license_key_hash: existing.license_key_hash,
        account_login: acc,
        revoked_at: new Date().toISOString(),
        reason: "Dihapus admin via Admin Panel",
      },
  ...
  const { error } = await supabase
    ...
    .update({
      revoked: true,
      revoked_at: new Date().toISOString(),
      pending_command: "REVOKE",
    })
    .eq("account_login", acc);
  if (error) throw error;
}

// Dipanggil dari ea-update.js SETELAH EA mengirim konfirmasi bahwa command
// REVOKE sudah dieksekusi (EA berhenti/keluar). Baru di titik ini baris
// `accounts` benar-benar dihapus permanen dari server. Baris di
// `revoked_licenses` TIDAK ikut terhapus di sini — itu memang permanen.
export async function purgeRevokedAccount(accountLogin) {
  if (!accountLogin) return;
  const acc = String(accountLogin);
  await supabase.from("journal_entries").delete().eq("account_login", acc);
  const { error } = await supabase.from("accounts").delete().eq("account_login", acc);
  if (error) throw error;
}

// Cek apakah sebuah hash kode lisensi ada di blacklist permanen. Dipanggil
// dari ea-update.js SEBELUM memproses sync apa pun (termasuk utk akun yang
// baru pertama kali terlihat / TOFU) — supaya kode lisensi yang pernah
// di-revoke tidak bisa dipakai lagi walau baris `accounts` lamanya sudah
// hilang (dihapus manual atau lewat alur purge normal).
export async function isLicenseRevoked(licenseKeyHash) {
  if (!licenseKeyHash) return false;
  const { data, error } = await supabase
...
// Cek apakah sebuah Akun ID (account_login) PERNAH punya lisensi yang
// di-revoke admin sebelumnya (tercatat di revoked_licenses.account_login).
// Dipakai HANYA saat REGISTRASI akun web baru — bukan saat sync EA:
//
//  - Akun ID yang belum pernah muncul sama sekali di revoked_licenses
//    (belum pernah dihapus admin) -> bebas daftar tanpa perlu EA sync
//    dulu, langsung "pending" menunggu admin approve.
//  - Akun ID yang PERNAH dihapus admin (ada baris account_login yg sama
//    di revoked_licenses) -> harus EA sync ULANG dulu (baris baru
//    muncul lagi di tabel `accounts`, otomatis berarti pakai kode
//    lisensi baru krn kode lama sudah di-blacklist di isLicenseRevoked)
//    sebelum boleh daftar akun web lagi utk Akun ID tsb.
//
// Catatan: satu account_login bisa punya >1 baris histori revoke (kalau
// dihapus admin berkali-kali dgn kode lisensi berbeda-beda), makanya pakai
// .limit(1) bukan .maybeSingle().
export async function isAccountEverRevoked(accountLogin) {
